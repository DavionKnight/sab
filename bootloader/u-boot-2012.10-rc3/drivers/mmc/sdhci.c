		       ...
		       struct mmc_data *data)
{
    ...
    debug("Start to send command, CMD ID:%d, data ptr:%.8x\n", cmd->cmdidx, (u32)data);
    ...
    debug("Send command result, stat:%.8x, mask:%.8x, ret:%d\n", stat, mask, ret);
...
static void sdhci_set_ios(struct mmc *mmc)
{
    ...
    debug("Set bus and clock, BUS:%d, Clock:%d\n", mmc->bus_width, mmc->clock);
...
int add_sdhci(struct sdhci_host *host, u32 max_clk, u32 min_clk)
{
    ...
    if (!caps) {
        debug("Get null capabilities, set the default value by manual\n");
        caps = 0x07eec8b0;
    }
    debug("Current host capabilities: %.8x\n", caps);
    ...
    debug("Host config, max clock:%d, min clock:%d\n", host->cfg.f_max, host->cfg.f_min);
