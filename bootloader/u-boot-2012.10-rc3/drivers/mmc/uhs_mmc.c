...
int mmc_startup(struct mmc *mmc)
{
	...
	if (IS_SD(mmc)) {
		...
		if (mmc->card_caps & MMC_MODE_4BIT) {
			printf("In %s card_cap 0x%x\n", __FUNCTION__, mmc->card_caps);
...
int mmc_register(struct mmc *mmc)
{
	...
	printf("In %s : mmc address : 0x%x\n", __FUNCTION__ ,mmc->block_dev.block_write);
	printf("In %s : mmc address : 0x%x\n", __FUNCTION__ ,mmc->block_dev.block_read);
	printf("In %s : mmc address : 0x%x\n", __FUNCTION__ ,mmc->block_dev.block_erase);
	if (!mmc->b_max)
